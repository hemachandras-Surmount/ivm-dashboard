...
export function SeverityChart({ data, title = "Vulnerability Distribution" }: SeverityChartProps) {
  ...
  const monthYears = Array.from(
    new Map(data.map((d) => [`${d.year}-${d.month}`, { month: d.month, year: d.year }])).values()
  ).sort((a, b) => a.year !== b.year ? a.year - b.year : (MONTH_ORDER[a.month] || 0) - (MONTH_ORDER[b.month] || 0));
  const latest = monthYears[monthYears.length - 1];
  const latestData = latest ? data.filter((d) => d.month === latest.month && d.year === latest.year) : data;
