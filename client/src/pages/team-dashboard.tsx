...
export default function TeamDashboard({ team }: TeamDashboardProps) {
  ...
  const monthlyData = data?.vulnerabilities.reduce((acc, v) => {
    ...
    if (!acc[key]) {
      acc[key] = { name: `${v.month} ${v.year}`, year: v.year, monthName: v.month, found: 0, resolved: 0 };
    }
    acc[key].found += v.count;
    acc[key].resolved += v.resolvedCount;
    return acc;
  }, {} as Record<string, { name: string; year: number; monthName: string; found: number; resolved: number }>) || {};

  const barChartData = Object.values(monthlyData)
    .sort((a, b) => a.year !== b.year ? a.year - b.year : (MONTH_ORDER[a.monthName] || 0) - (MONTH_ORDER[b.monthName] || 0))
    .slice(-3);
        ...
        {isLoading ? (
          ...
          <StatsOverview 
            ...
            progressBarVisibility={{
              asset_coverage: sectionSettings.isVisible("pb_asset_coverage"),
              compliance_score: sectionSettings.isVisible("pb_compliance_score"),
              findings_resolution: sectionSettings.isVisible("pb_findings_resolution"),
            }}
            onToggleProgressBar={(key, visible) => sectionSettings.toggleVisibility(`pb_${key}`, visible)}
          />
        ) : null}
      </AdminSectionWrapper>
