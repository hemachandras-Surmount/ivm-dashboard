...
export default function Report() {
  ...
  const renderTeamReport = (
    ...
    const cardH = 28;
    const isAppSec = rpt.team === 'application';
    const summaryCards = [
      ...
      ...(!isAppSec ? [{ label: 'Avg. MTTR', value: `${rpt.mttr.toFixed(1)} days`, highlight: false }] : []),
      { label: 'Remediation Rate', value: `${rpt.remediationRate}%`, highlight: rpt.remediationRate < 70 },
    ];

    const cardW = (contentW - (summaryCards.length - 1) * 3) / summaryCards.length;
    checkPage(cardH + 5);
    ...
    const assetCardsAll = [
      { label: isAppSec ? 'Total Applications' : 'Total Assets', value: rpt.stats.totalAssets.toLocaleString(), highlight: false },
      ...
      ...(!isAppSec ? [{ label: 'Risk Score', value: `${rpt.riskScore.toFixed(1)}/10`, highlight: rpt.riskScore > 7 }] : []),
      { label: 'Compliance', value: `${(rpt.complianceScore ?? rpt.stats.complianceScore ?? 0).toFixed(1)}%`, highlight: false },
    ];
    const cardW5 = (contentW - 12) / assetCardsAll.length;

    assetCardsAll.forEach((card, i) => {
    ...
    if (rpt.monthlyFindings && rpt.monthlyFindings.length > 0) {
      ...
      rpt.monthlyFindings.forEach((mf, i) => {
        ...
        doc.setTextColor(...muted);
        doc.setFont('helvetica', 'normal');
        doc.text(`${mf.month} ${mf.year}`, cx, mfBarBase + 5, { align: 'center' });
      });
  ...
  const handleExportPdf = async () => {
    setIsExporting(true);
    try {
      const res = await fetch(`/api/report/${selectedTeam}?t=${Date.now()}`, { cache: 'no-store' });
  ...
  const handleExportConsolidatedPdf = async () => {
    ...
    try {
      ...
      for (const team of TEAMS) {
        const res = await fetch(`/api/report/${team.value}?t=${Date.now()}`, { cache: 'no-store' });
          ...
          <Card className="print:shadow-none print:border-2">
            ...
            <CardContent className="pt-6 space-y-8">
              ...
              <section>
                ...
                <div className={`grid grid-cols-2 ${report.team === "application" ? "md:grid-cols-4" : "md:grid-cols-5"} gap-4`}>
                  <StatBox label={report.team === "application" ? "Total Applications" : "Total Assets"} value={report.stats.totalAssets} />
