...
export async function registerRoutes(
  ...
  app.get("/api/report/:team", async (req, res) => {
    ...
    try {
      ...
      const totalFound = Object.values(vulnSummary).reduce((sum, s) => sum + s.total, 0);
      const totalResolved = Object.values(vulnSummary).reduce((sum, s) => sum + s.resolved, 0);
      const totalOpen = Object.values(vulnSummary).reduce((sum, s) => sum + s.open, 0);
      ...
      const report = {
        ...
        totalClosedFindings: totalResolved,
        remediationRate: totalFound > 0 ? 
          Math.round((totalResolved / totalFound) * 100) : 0,
        mttr: stats?.mttr || 0,
